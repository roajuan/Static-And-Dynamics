...
for t=dt:dt:tmax
 ...
 g = (G*M_earth)/((z*cos(theta1)+6371000)^2); % gravitational acceleration, g(z)
 m = m - (dm*dt); % changing mass, m(t)
 [rho,temp,press] = density(z*cos(theta1));
 % changing air density by barometric formula, rho(z)
 Cd = CD(v*sin(theta1),temp,C0);
 thrust = (FT/m)*cos(theta1);
 drag = (0.5*rho*(v^2)*Cd*A/m)*cos(theta1);


 v = v*cos(theta1) + (thrust - drag-g*cos(theta1))*dt; % new velocity
 z = z*cos(theta1) + v*dt*cos(theta1); % new altitude
...
subplot(2,1,1)
plot(t,Z/1000)
title('Rocket displacement on x')
ylim([0,1.5*max(Z)/1000])
xlabel('time (s)')
ylabel('displacement on x(km)')
subplot(2,1,2)
plot(t,V)
title('Rocket velocity on x')
ylim([1.5*min(V),1.5*max(V)])
xlabel('time (s)')
ylabel('velocity on x (m/s)')
%% Plot the forces
figure
subplot(3,1,1)
